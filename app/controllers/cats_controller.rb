class CatsController < ApplicationController
  ...
  def create
    ...
    logger.info("cat creation 222")

    @cat = Cat.new(cat_params)
    @cat.user_id = current_user.id

    logger = Logger.new("my_log.txt")
    logger.info("cat creation 111")

    if @cat.save
      redirect_to cat_url(@cat)
    else
      logger = Logger.new("my_log.txt")
      logger.info("cat creation failed")
      flash.now[:error] = @cat.errors
      render :new
    end
  end
  ...
  def cat_params
    params.require(:cat).permit(:name, :birth_date, :color, :sex, :description, :image_url)
  end
end